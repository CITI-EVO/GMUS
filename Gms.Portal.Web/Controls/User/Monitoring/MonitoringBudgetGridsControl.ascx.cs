...
{
    public partial class MonitoringBudgetGridsControl : BaseUserControl
    {
        ...
        public void InitStructure(ControlEntity entity)
        {
            ...
            var budgetEntity = controlLp["ProjectBudget"].SingleOrDefault();
            if (budgetEntity == null)
                errors.Add(@"Unable to find 'ProjectBudget'");
            else
        ...
        private void SetGrid(Control parent, GridEntity entity)
        {
            ...
            if (entity.Controls != null)
            {
                ...
                foreach (var field in orderedVisibleFields)
                {
                    ...
                    if (field.DependentFieldID != null)
                        SetVisibility(column, field);
        ...
        private void ProcessDependencies(FormDataUnit formData)
        {
            foreach (var entity in Entities.Values)
            {
                var fields = FormStructureUtil.InOrderTraversal(entity);
                foreach (var field in fields)
                {
                    if (field.DependentFieldID == null)
                        continue;

                    var parentKey = Convert.ToString(field.DependentFieldID);
                    var parentVal = formData[parentKey];

                    var dependentExp = field.DependentExp;
                    if (String.IsNullOrWhiteSpace(dependentExp))
                        dependentExp = "!IsEmpty(@value)";
        ...
        private void SetVisibility(GridViewMetaBoundField column, FieldEntity field)
        {
