...
{
    public partial class UserExpertDataGridControl : BaseUserControlExtend<FullExpertDataGridModel>
    {
        ...
        protected String GetFormExperts(object dataItem)
        {
            ...
            var statusUnits = descriptor.GetValue(FormDataConstants.UserStatusesFields) as IEnumerable<FormStatusUnit>;
            if (statusUnits == null)
                return String.Empty;
            ...
            if (String.IsNullOrWhiteSpace(entity.SelectorExpression))
                return String.Empty;


            var validStatusUnits = (from n in statusUnits
                                    where n.StatusID == DataStatusCache.Submit.ID
                                    select n).ToList();

            var statusUnitsLp = validStatusUnits.ToLookup(n => n.UserID);

            var scoresList = GetRecordScores(validStatusUnits, entity).ToList();
            var advResolver = new AdvancedDataResolver { ["scores"] = scoresList };
            ...
            var selectorData = selectorResult.Value as IEnumerable;
            if (selectorData == null)
                return String.Empty;

            var selectorList = selectorData.Cast<IDictionary<String, Object>>();

            var selectorStatusUnits = (from n in selectorList
                                       let v = DataConverter.ToNullableGuid(n.GetValueOrDefault("UserID"))
                                       let s = statusUnitsLp[v].FirstOrDefault()
                                       where s != null
                                       select s);

            var experts = (from n in selectorStatusUnits
                           where n.Params != null &&
                                 n.Params.ContainsKey(FormDataConstants.ScoringParams)
