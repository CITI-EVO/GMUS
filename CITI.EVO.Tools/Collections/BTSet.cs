...
{
    public class BTSet<TItem> : ISet<TItem>
    {
        private readonly BTree<TItem> _btree;

        public BTSet() : this(Comparer<TItem>.Default)
        {
        }
        public BTSet(IComparer<TItem> comparer)
        {
        ...
        {
            foreach (var item in _btree.TraversalItems())
                yield return item;
        }
