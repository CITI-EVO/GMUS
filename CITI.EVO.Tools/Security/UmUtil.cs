    ...
    public class UmUtil : ISerializable, IDisposable
    {
        ...
        public IDictionary<String, String> CurrentUserAttributes
        {
            get
            {
                ...
                if (currentUserAttributes == null)
                {
                    ...
                    foreach (var userGroup in CurrentUserGroups)
                    {
                        var groupAttrs = UserManagementProxy.GetUserAttributesDictionary(CurrentToken.Value, userGroup.ID, ProjectID.Value);

                        foreach (var pair in groupAttrs)
                        {
                            var value = currentUserAttributes.GetValueOrDefault(pair.Key);
                            ...
                            currentUserAttributes[pair.Key] = value;
                        }
                    }
                }
        ...
        public void PreloadUserPermissions()
        {
            ...
            foreach (var permsByProjectGrp in permsByProjects)
            {
                ...
                foreach (var permsByPathGrp in permsByPathsLp)
                {
                    ...
                    var path = $"{permsByProjectGrp.Key}/{permsByPathGrp.Key}";
        ...
        private void LogEvent(String eventName, String loginName, String password, Guid? token, bool? success)
        {
            if (Logger != null)
            {
                ...
                var logText =
                    $"Event: {eventName}; LoginName: {loginName}; Password: {password}; ProjectID: {ProjectID}; Token: {token}; SessionID: {sessionID}; ClientIP: {clientIP}; Success: {success}";
